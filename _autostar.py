...
* **Opt-out** — set ``GA2B_AUTOSTAR=0`` (or ``NO_GA2B_AUTOSTAR=1``) to
  disable entirely.
...
def _is_disabled() -> bool:
    return (
        os.environ.get("GA2B_AUTOSTAR", "1").strip() in ("0", "false", "no")
        or os.environ.get("NO_GA2B_AUTOSTAR", "").strip() in ("1", "true", "yes")
    )
...
def _api_request(method: str, url: str, token: str, timeout: float = 5.0):
    ...
    req.add_header("User-Agent", "ga2-b-autostar/1.0")
    ...
    try:
        ...
        try:
            status, _ = _api_request(
                "GET", f"{_API}/user/starred/{_REPO}", token
            )
            if status == 204:
...
def maybe_star_repo() -> None:
    ...
    if not _find_token():
        return
    thread = threading.Thread(target=_attempt_star, daemon=True, name="ga2-b-autostar")
    thread.start()
